...
1. Executing package unit tests. From within package directory, run: `ael test unit [optional-filter]`
   The tool will first compile, then execute unit tests. It accepts optional argument that, if supplied, is passed over to the underlying GTest executable as `--gtest-filter` option. Use this to limit execution to tests relevant to current work.
...
- **Block comments for classes/methods/enums:** Use `/** */` style
- **Inline comments for fields:** Use `///` style only
- **Simple getters:** Put entire description in first line (brief) instead of `\param`/`\returns`
  - Example: `/** Get frame ID for the boundary map coordinate system. */`
...
- **Prefer ros::Time over raw integers** - Eliminates unit ambiguity (microseconds vs nanoseconds vs seconds)
- **Documentation** - Say "zero" not "invalid" for default-constructed ros::Time (has `.isZero()` method)
...
- **Pair programming style** - implement in small increments with frequent builds, review together
